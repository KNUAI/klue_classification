...
parser.add_argument('--train_files', default='./KLUE/klue_benchmark/ynat-v1.1/ynat-v1.1_train.json', type=str, help='train_files_dir')
parser.add_argument('--valid_files', default='./KLUE/klue_benchmark/ynat-v1.1/ynat-v1.1_dev.json', type=str, help='valid_files_dir')
...
input_data = load_dataset('klue', 'ynat')
def read_data(tokenizer, paths, files):
    ...
    for data in data_list:
        title, label = data['title'], data['label']
        titles.append(title)
        labels.append(label)

    #label_list
    label_list = list(set(labels))
    #label_list = ['정치', '경제', '사회', '생활문화', '세계', 'IT과학', '스포츠']
    ...
    for i in range(len(data_list)):
        input_dict = tokenizer(titles[i], padding = 'max_length', truncation = True, max_length = args.max_length, return_tensors = 'pt', return_attention_mask = False)
...
def read_data(tokenizer, paths, files):
    ...
    targets = []
    for i in range(len(data_list)):
        input_dict = tokenizer(titles[i], padding = 'max_length', max_length = args.max_length, return_tensors = 'pt', return_attention_mask = False)
        inputs.append(input_dict['input_ids'])
        segs.append(input_dict['token_type_ids'])
        targets.append(label_map[labels[i]])
